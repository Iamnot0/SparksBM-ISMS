...
public class ClientChangedEventListener {
  ...
  private void createClient(ClientChangedEvent event) {
    ...
    if (repository.exists(event.getClientId())) {
      log.warn("Client already exists: {}", event.getClientId());
    ...
    log.info("Created client entity: id={}, name={}", client.getIdAsString(), client.getName());
    ...
    log.info("Client state updated to: {}", client.getState());
    addDomains(client, event.getDomainProducts(), false);
    addProfiles(client, event.getDomainProducts(), false);
    Client saved = repository.save(client);
    log.info("Client saved successfully: id={}, name={}, state={}", saved.getIdAsString(), saved.getName(), saved.getState());
  }
