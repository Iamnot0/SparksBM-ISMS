...
function useUrl() {
  ...
  const requestParams = computed(() => {
    ...
    const domainId = Array.isArray(domainParam) ? domainParam[0] : (typeof domainParam === 'string' ? domainParam : '');
    const objectTypeParam = route.params.objectType;
    const elementType = Array.isArray(objectTypeParam) ? objectTypeParam[0] : (typeof objectTypeParam === 'string' ? objectTypeParam : '');
    const objectParam = route.params.object;
    const elementId = Array.isArray(objectParam) ? objectParam[0] : (typeof objectParam === 'string' ? objectParam : '');
    return {
      domainId,
      elementType,
      elementId
