...
function getItemField(item: any, field: 'name' | 'description'): string {
  ...
  // For profiles (items with domainId), use direct property
  if (item.domainId) {
    return item?.[field] || '';
  }
  
  // For domains and other items with translations, prefer translations but fallback to direct property
  const translatedValue = item?.translations?.[locale.value]?.[field];
  if (translatedValue) {
    return translatedValue;
  }
  
  // Fallback to direct property (for domains that have name/description directly)
  return item?.[field] || '';
}
