...
import type { IVeoReportsMeta, IVeoReportMeta, IVeoFetchReportsParameters } from '~/composables/api/queryDefinitions/reports';
      ...
      return {
        ...
        children: [
          // all of object type
          {
            ...
            to: props.unitId && props.domainId && VeoElementTypePlurals[modelType] ? {
...
// Fetch reports for navigation (reports are global, don't need domainId)
const fetchReportsQueryParameters = computed<IVeoFetchReportsParameters>(() => ({}));
const { data: reports, isFetching: reportsLoading } = useQuery(
  reportQueryDefinitions.queries.fetchAll,
  fetchReportsQueryParameters,
  { enabled: true } // Always enabled - reports are global
);
...
// Reports child items - individual report types
const reportChildItems = computed<INavItem[]>(() => {
  if (!reports.value) return [];
  
  const reportsMeta = reports.value as IVeoReportsMeta;
  const localeValue = locale.value;
  
  return Object.entries(reportsMeta).map(([reportId, reportMeta]: [string, IVeoReportMeta]) => {
    // Get report name in current locale, fallback to English, then to reportId
    const reportName = reportMeta.name?.[localeValue] || 
                       reportMeta.name?.['en'] || 
                       reportMeta.name?.[Object.keys(reportMeta.name || {})[0]] ||
                       reportId;
    
    return {
      id: reportId,
      name: reportName,
      componentName: `report-${reportId}`,
      to: {
        name: 'unit-domains-domain-reports-report',
        params: {
          unit: props.unitId,
          domain: props.domainId,
          report: reportId
...
const reportsNavEntry = computed<INavItem>(() => ({
  ...
  children: reportChildItems.value,
  childrenLoading: reportsLoading.value,
...
const items = computed<INavItem[]>(() => [
  ...(props.unitId && props.domainId ?
    [
      ...
      reportsNavEntry.value, // Always show reports (even if empty, it will show when backend responds)
      ...(!isEmpty(catalogsEntriesChildItems.value) ? [catalogsNavEntry.value] : []),
      ...(!isEmpty(riskChildItems.value) ? [risksNavEntry.value] : []) // Show risks if domain has risk definitions
    ]
  : [])
]);
