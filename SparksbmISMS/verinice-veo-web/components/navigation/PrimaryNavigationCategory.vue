...
const onClick = () => {
  ...
  if (props.to) {
    try {
      ...
      if (resolvedRoute && resolvedRoute.name && route.path !== resolvedRoute.fullPath) {
        router.push(props.to).catch((error) => {
          console.error('Navigation failed:', error);
        });
      }
    } catch (error) {
      console.error('Route resolution failed during navigation:', error);
    }
  }
};
