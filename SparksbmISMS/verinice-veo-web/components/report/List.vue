...
const fetchDomainQueryEnabled = computed(() => {
  const domainParam = route?.params.domain;
  const domainId = Array.isArray(domainParam) ? domainParam[0] : (typeof domainParam === 'string' ? domainParam : '');
  return !!domainId;
});
...
function filterReportsByDomain(_domain: IVeoDomain, _allReports: IVeoReportsMeta) {
  if (!_allReports) return [];
  
  // If domain is missing, return all reports (reports are global)
  if (!_domain) {
    return Object.entries(_allReports);
  }
...
const displayedItems = computed(() => {
  ...
  // Filter reports (will return all if domain is missing)
