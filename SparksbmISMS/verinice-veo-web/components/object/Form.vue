...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const getFormschemaIdBySubType = (subType: string) => {
      const formSchemaId = (formSchemas.value && Array.isArray(formSchemas.value) ? 
    ...
    const dataIsLoading = computed<boolean>(
      () => {
        // Only show loading if critical data is missing
        // Form schemas and translations are optional, so don't block on them
        const criticalLoading = 
        objectSchemaIsFetching.value ||
        props.loading ||
          domainIsFetching.value;
        
        // If we have objectSchema and domain, we can show the form
        // even if form schemas/translations are still loading
        if (objectSchema.value && domain.value && !criticalLoading) {
          return false;
        }
