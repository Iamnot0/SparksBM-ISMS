...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const { data: domain, isFetching: domainIsFetching, error: domainError } = useQuery(
      ...
      fetchDomainQueryParameters,
      {
        ...
        onError: (error) => {
          ...
          displayErrorMessage('Failed to load domain data. Please refresh the page.');
        },
    ...
    const seedInitialData = () => {
      if (!domain.value) {
        return; // Wait for domain to load
      }
      ...
      if (props.domainId && props.subType) {
        ...
        // Get default status for this subtype
        const elementDef = domain.value.elementTypeDefinitions?.[props.objectType];
        const subTypeDef = elementDef?.subTypes?.[props.subType];
        const statuses = subTypeDef?.statuses || [];
        objectData.value.status = statuses[0] || 'NEW';
      } else if (props.domainId && domain.value) {
        // If no subtype preselected, use first available subtype
        const elementDef = domain.value.elementTypeDefinitions?.[props.objectType];
        const subTypes = elementDef?.subTypes;
        if (subTypes && Object.keys(subTypes).length > 0) {
          const firstSubType = Object.keys(subTypes)[0];
          objectData.value.subType = firstSubType;
          const statuses = subTypes[firstSubType]?.statuses || [];
          objectData.value.status = statuses[0] || 'NEW';
        }
      }
    ...
    const { mutateAsync: create } = useMutation(objectQueryDefinitions.mutations.createObject, {
      onSuccess: async (queryClient, data: IVeoAPIMessage) => {
        ...
        // Use debounced invalidation to avoid multiple rapid refetches
        if (props.parentScopeIds) {
          await nextTick(); // Wait for dialog to start closing
          // Batch invalidations to reduce refetch overhead
          const invalidations = [];
          for (const scope of props.parentScopeIds) {
            invalidations.push(
              queryClient.invalidateQueries(['object', { endpoint: 'scopes', id: scope }]),
              queryClient.invalidateQueries(['childObjects', { endpoint: 'scopes', id: scope }]),
              queryClient.invalidateQueries(['childScopes', { id: scope }])
            );
          }
          // Execute invalidations in parallel but don't wait
          Promise.all(invalidations).catch(() => {}); // Ignore errors, queries will refetch on next access
        }
